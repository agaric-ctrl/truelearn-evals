"""Tier 3: runs the source-coverage judge against
maestro/judge/fixtures/source_coverage_cases.json and reports whether it agrees with each case's
...
Run:  python3 maestro/judge/run_gold_suite_source_coverage.py [--json]
...
DEFAULT_FIXTURE = Path(__file__).parent / "fixtures" / "source_coverage_cases.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Maestro source-coverage judge against its synthetic gold suite.",
    )
    ...
    evaluation = EvaluationResult(
        framework="maestro",
        metric="source_coverage_experimental_unvalidated",
        cases=cases,
