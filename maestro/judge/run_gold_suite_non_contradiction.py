"""Tier 3: runs the non-contradiction judge against
maestro/judge/fixtures/non_contradiction_cases.json and reports whether it agrees with each case's
known (hand-crafted, not SME-verified) expected_pass.

Same discipline as run_gold_suite.py: tests the JUDGE, not a generator.
...
Run:  python3 maestro/judge/run_gold_suite_non_contradiction.py [--json]
...
from tools.eval_result import EvaluationResult
from tools.provider_usage import estimate_cost

BANNER = (
    "=" * 78 + "\n"
    "EXPERIMENTAL - this judge is validated only against 4 hand-crafted synthetic cases,\n"
    "NOT real SME-graded data. Its agreement with real SME judgment has not been measured.\n"
    "Not evidence. Not wired into any gate.\n" + "=" * 78
)

DEFAULT_FIXTURE = Path(__file__).parent / "fixtures" / "non_contradiction_cases.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the Maestro non-contradiction judge against its synthetic gold suite.",
    )
    ...
    evaluation = EvaluationResult(
        framework="maestro",
        metric="non_contradiction_experimental_unvalidated",
        cases=cases,
