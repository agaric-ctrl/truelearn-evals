...
## Note added Sep 9 — check before building

There's an unresolved conflict (see `CLAUDE.md`'s URGENT section and findings doc Section 7) between a live meeting claiming semantic-search/RAG-style retrieval is now grounding article generation, and a same-day Data Science comment stating prompt-merging is still current and RAG is only a benchmark candidate. This task's judges are designed around the "articles are not RAG-sourced, retrieved content constrains rather than sources" framing. **Confirm which is actually true before finalizing judge prompts** — if RAG-style retrieval has genuinely shipped, the non-contradiction judge's design (built against prompt-merging assumptions) may need a light revisit, though the core mechanism (custom prompt vs. framework-native metric) likely still holds either way.
